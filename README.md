...
- [`testdata/native/token-classification/`](./testdata/native/token-classification) contains the first checked-in native token-classification bundle
...
- the native `bionet` path is now also validated on a public `dslim/distilbert-NER` token-classification reference set without `libtorch`
...
1. widen the token-classification reference set beyond the initial NER smoke test
2. decide whether the native token-classification path needs a small context feature before any attention-style work
3. decide whether layer normalization is ready to graduate from experimental generator support into the default native parity path
